defmodule Egregoros.Activities.Announce do
  ...
  defp validate_inbox_target(%{} = activity, opts) when is_list(opts) do
    InboxTargeting.validate_addressed_or_followed_or_object_owned(
      ...
      extract_object_id(Map.get(activity, "object"))
    )
  end
  ...
  defp apply_announce(activity, %__MODULE__{} = announce) do
    ...
    |> Helpers.maybe_put("to", announce.to)
    |> Helpers.maybe_put("cc", announce.cc)
    |> Helpers.maybe_put("published", announce.published)
  end
